...
첫번째 열의 정보는 job 번호를 의미한다.

중지되거나 백그라운드에서 돌고 싶은 작업이 있을 때 이 job 번호를 이용해서 실행시킨다.
...
+는 fg나 bg명령어를 쳤을 때 가장 먼저 가져와서 수행하게 될 프로세스를 의미하고,

- 여기서 fg는 백그라운드 프로세스를 포어그라운드로 전환하는 명령어이고

- bg는 포어그라운드 프로세스를 백그라운드로 돌리는 명령어이다.
-는 현재 진행중인 job이 끝나면 바로 다음에 수행될 프로세스를 의미한다.
---
3. 세번째 열의 정보

세 번째 열에 보면 stopped를 볼 수 있는데 이 표시는 job 상태를 의미합니다.
...
kill 명령어는 프로세스에 시그널을 보내는 명령어로 시그널을 받은 프로세스의 기본 동작이 종료이기 때문에 kill이라고 명명되었다.

___아래 있는 목록이 모두 kill명령어의 옵션이다___
<img src = "https://user-images.githubusercontent.com/97230837/172001205-e0a4542d-f2b1-48ff-afb9-d0c13fb658d9.PNG" width = "700" height="400">

1. 사용방법

kill -[옵션][PID]

가장 대표적인 kill의 옵션인 9번 강제 종료를 사용해보자면

ex) kill -9 [PID]
```
PID에 해당하는 프로세스를 강제 종료 하여라
```
- 여기서 PID는 위에 나와있던 ps 명령어로 알아낼 수 있다.
...
kill 명령어를 통해 프로세스를 강제 종료하면 오류가 생길 수 있으므로

>process에 문제가 있어서 다시 시작하고자 할 때에는 SIGHUP, SIGTERM, SIGKILL의 순서로 각각 시도하는 것을 권장한다.

## vim 에디터 매크로

1. 매크로를 기록하는 방법

vim에디터에서 중립모드로 들어간뒤 q를 누른 다음 매크로 이름으로 사용할 알파벳을 눌러준다.

그럼 밑에 모드표시하는 곳에 --recording--이라고 뜰 때 사용할 매크로를 키보드로 입력해준다.
...
중립모드에서 @+[본인이 설정해놓은 알파벳]을 누르게되면 매크로가 재생이된다.

@@를 누르면 제일 마지막에 재생된 매크로가 재생된다.

{반복횟수}@{본인이 설정해놓은 알파벳}` 또는 `{반복횟수}@@` 로 저장된 매크로를 '반복횟수' 만큼 재실행을 하게된다.
...
자동으로 "hello wolrd"가 입력됨

- 3@a를 하게되면
"hello world""hello world""hello world"가 입력됨
```
---
3.매크로를 파일로 저장하는법.

자신이 자주사용하는 매크로이거나 만들기 어려운 매크로라면 버리기 아까운 경우가 있을 것이다.

이럴때 (vim ~/.vimrc) 명령어를 입력하여 vimrc파일을 열어

let @{사용하고 싶은 알파벳} = {매크로 내용}을 입력하면 저장이된다.

이를 사용할때는 vim에디터 입력 모드에서 ctrl+R을 누르고 {본인이 지정한 알파벳}을 누르게 되면 매크로가 실행된다.

```
ex)hello world를 출력하는 매크로 만들어 저장하기

vim ~/.vimrc

<vimrc 내부>
let @a = "hello world"을 저장한뒤
